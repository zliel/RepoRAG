...
from code_navigator.llm.prompts import (
    ANSWER_SYSTEM,
    DIAGRAM_SYSTEM,
    REWRITE_SYSTEM,
    build_context_block,
...
from code_navigator.parsing.python_chunks import extract_chunks
from code_navigator.retrieval.search import RetrievedChunk, top_k_similar
...
def _retrieve_hits(
    ...
    try:
        ...
        idx.close()
    if mat.size == 0:
        typer.echo("Index is empty. Run `code-nav index` first.", err=True)
        raise typer.Exit(1)
